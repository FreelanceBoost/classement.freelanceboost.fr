class DribbblersController < ApplicationController
  ...
  def sync_es(dribbler)
    ...
    response = client.search index: 'influencers', body: { min_score: 0.9, query: { match: { pseudo: dribbler.username } } }
    result =  Hashie::Mash.new response
    if result.hits.total > 0
      user = result.hits.hits[0]._source
    else
      response = client.search index: 'influencers', body: { min_score: 0.9, query: { match: { name: dribbler.name } } }
      result = Hashie::Mash.new response
      if result.hits.total > 0
        user = result.hits.hits[0]._source
      else
