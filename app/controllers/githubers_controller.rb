class GithubersController < ApplicationController
  ...
  def sync_es(githuber)
    ...
    response = client.search index: 'influencers', body: { min_score: 0.9, query: { match: { pseudo: githuber.github_login } } }
    result = Hashie::Mash.new response
    if result.hits.total > 0
      user = result.hits.hits[0]._source
    else
      if githuber.name
        response = client.search index: 'influencers', body: { min_score: 0.9, query: { match: { name: githuber.name } } }
      else
        response = client.search index: 'influencers', body: { min_score: 0.9, query: { match: { pseudo: githuber.github_login } } }
      end
      result = Hashie::Mash.new response
