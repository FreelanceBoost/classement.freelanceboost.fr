...
class RockstarsController < ApplicationController
  ...
  def sync_es(rockstar)
    ...
    response = client.search index: 'influencers', body: { min_score: 1, query: { match: { pseudo: rockstar.pseudo } } }
    result = Hashie::Mash.new response
    if result.hits.total > 0
      user = result.hits.hits[0]._source
    else
      response = client.search index: 'influencers', body: { min_score: 1, query: { match: { name: rockstar.name } } }
      result = Hashie::Mash.new response
      if result.hits.total > 0
        user = result.hits.hits[0]._source
      else
